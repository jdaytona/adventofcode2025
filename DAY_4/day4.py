...
def part1_get_neighbors(input_grid, total_count):
    # print(grid[1][3])
    ...
    for row in grid_row_range:
        for column in grid_column_range:
            ...
            if neighbors and neighbors.count(lookup_item) < 4:
                # print(f'{row}{column} - {neighbors}')
                # print(f'{row}{column} adjacent elements are {neighbors}')
...
def part2_get_neighbors(input_grid, total_count):
      # print(grid[1][3])
    grid_rows       = len(input_grid)
    grid_columns    = len(input_grid[0])
...
with open('/Users/jeremy/adventofcode2025/DAY_4/day4_input.txt', mode='r') as f:
    ...
    part1_total_count = part1_get_neighbors(grid, part1_total_count)
    print(f'PART 1 - ToTAL COUNT : {part1_total_count}')
    new_grid, part2_total_count, count, roles_to_remove = part2_get_neighbors(grid, part2_total_count)
    ...
    while count:
        for i in roles_to_remove:
            ...
            y = int(i.split(':')[1])
            new_grid[x][y] = '.'
        new_grid, part2_total_count, count, roles_to_remove = part2_get_neighbors(new_grid, part2_total_count)
